...
import { MediaControlSongListBreadCrumbs } from "@/constants/bread-crumbs";
...
import { MediaControlSongListHeadings } from "@/constants/table-headings";
...
export default function MediaControlSongList() {
    ...
    const buildApiUrl = () => {
        ...
        return `${baseURL}/media-control/song?${params.toString()}`;
    };
    ...
    const handleDeleteSong = async (songId: string) => {
        if (!isOnline) {
            toast.error("Cannot delete song while offline");
            return;
        }
        try {
            mutate(
                (currentData: ApiResponse | undefined) => currentData ? {
                    ...currentData,
                    SONGS: currentData.SONGS.filter(song => song._id !== songId)
                } : currentData,
                false
            );
            const response = await fetch(`${baseURL}/media-control/song/${songId}`, {
                method: 'DELETE',
                credentials: 'include'
            });
    ...
    const filteredSong = data?.SONGS?.filter(song =>
    ...
    return (
        <div className="flex flex-1 flex-col bg-slate-100">
            <div className="align-center flex justify-between p-4 border-b shadow-md bg-white">
                <h1 className="text-sm font-bold text-gray-800">MANAGE SONGS</h1>
                <CustomBreakCrumb data={MediaControlSongListBreadCrumbs} />
            </div>
            <div className="flex flex-1 flex-col m-4">
                ...
                >
                    <div className="flex flex-wrap items-center m-2 gap-4 md:flex-2">
                        ...
                        <button className="bg-purple-700 px-10 py-1.5 ml-2 rounded-sm flex items-center transition duration-300 ease-in-out hover:bg-opacity-80" onClick={handleSearch}>
                    ...
                    <div className="overflow-x-auto grid grid-cols-1">
                        <Table>
                            <TableHeader>
                                <TableRow>
                                    {MediaControlSongListHeadings.map((heading, index) => {
                            ...
                            {isLoading ? (
                                <TableBody>
                                    <TableRow>
                                        <TableCell colSpan={MediaControlSongListHeadings.length} className="text-center py-6">
                                    ...
                                    <TableRow>
                                        <TableCell colSpan={MediaControlSongListHeadings.length} className="text-center py-6">
                                            <div className="flex flex-col items-center justify-center gap-2">
                                                ...
                                                <p className="text-gray-500">Unable to load song data</p>
                                                <button
                                                    onClick={() => mutate()}
                                ...
                                <TableBody>
                                    <TableRow>
                                        <TableCell colSpan={MediaControlSongListHeadings.length} className="text-center py-6">
                                            <NotFound />
                                            <p className="text-gray-500 mt-2">No Song found</p>
                                        </TableCell>
                                    </TableRow>
                                </TableBody>
                            ) : (
                                <TableBody>
                                    {filteredSong.map((song, index) => (
                                        <TableRow key={song._id} className="hover:bg-gray-50">
                                            ...
                                            <TableCell className="text-sm">
                                                <div className="flex items-center">
                                                    ...
                                                    <span className="font-normal">{song.name}</span>
                                                </div>
                                            </TableCell>
                                            <TableCell className="text-sm font-normal">
                                                {song.duration}
