...
import { MediaControlCommentDetailsBreadCrumbs } from '@/constants/bread-crumbs'
...
export default function MediaControlCommentDetails() {
    const { slug } = useParams()
    const [currentTab, setCurrentTab] = useState<'likes' | 'comments'>('likes')
    return (
        <>
            <div className="flex justify-between p-4 bg-white">
                <h1 className="text-sm font-bold text-gray-700">COMMENT DETAILS</h1>
                <CustomBreadCrumb data={MediaControlCommentDetailsBreadCrumbs} />
            </div>
            <div className="p-5 bg-slate-100">
                <div className="grid lg:grid-cols-[0.75fr_2.25fr] grid-cols-1 gap-5">
                    <div className="flex flex-col space-y-2">
                        <CardWrapper name={`Comment ${slug}`}>
                            <div className="flex flex-col items-center justify-center text-sm p-2">
                                <img src="https://placehold.co/150x150" alt="comment" className="rounded-full w-24 h-24 border-black border-double" />
                                <span> Commented By : Rakshak Khandelwal</span>
                            </div>
                            <div className="bg-gray-50 m-3 p-4 space-y-2">
                                <div className="font-semibold text-slate-800 py-2">Comment Information</div>
                    ...
                    <div className="flex flex-col space-y-2">
                        <CardWrapper name="Comment Content" icon={<Trash2 size={24} className='text-red-500 hover:text-red-600' onClick={()=> alert("Deleted")} />}>
                            <div className="flex flex-col justify-center items-center space-y-4 p-4">
                                ...
                                <div className=" w-1/2 p-2 rounded-md flex justify-between items-center border">
                                    <CommentBox />
                                </div>
                            </div>
                        </CardWrapper>
