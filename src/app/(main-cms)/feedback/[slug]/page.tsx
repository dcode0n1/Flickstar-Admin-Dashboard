...
export default function SupportTicketReply() {
    ...
    const { data, error, isLoading, mutate } = useSWR(`${baseURL}/setting/ticket/${params.id}`, getFetcher)
    ...
    const closeTicket = async () => {
        try {
            const response = await axios.patch(`${baseURL}/setting/ticket/${params.id}`, {}, {
    ...
    const onSubmit: SubmitHandler<SupportTicketData> = async (data) => {
        ...
        try {
            ...
            const response = await axios.put(`${baseURL}/setting/ticket/${params.id}`, { ...rest, ...attachments }, {
    ...
    return (
        <div className="flex flex-1 flex-col bg-slate-100">
            <div className="align-center flex justify-between p-4 border-b shadow-md bg-white">
                <h1 className="text-sm font-bold text-gray-800">FEEDBACK REPLY</h1>

            </div>
            <div className="flex flex-1 flex-col m-4">
                <CardWrapper name="Feedback Reply">
                    <div className="p-4">
                        <div className="p-2 bg-slate-100 space-y-2 max-h-[60vh] overflow-y-auto">
                            <div className="space-x-2">
                                ...
                                <span className="text-xs">Feedback Id - {data?.SUPPORTTICKET?.ticketId}</span>
                            </div>
